...
// A sale-scale "market" construction: 3 multi-token labels plus one one-word
// label, all carrying the dictionary token "market". At registration-scale
// minimums (themeMin=12) this is dropped entirely (v["n"] < 12); at sales-scale
// minimums (themeMin=3, memberMin=3, independentRootsMin=3, minTokens=1,
// including single-token labels) it clears every gate.
...
test('registration-scale defaults drop a sales-scale (4-label) construction entirely', async (t) => {
  ...
  for (const bucket of ['rising', 'new', 'stable', 'fading', 'unranked']) {
    assert.ok(!(tc[bucket] || []).some(r => r.theme === 'market'),
      `bucket ${bucket} must not contain "market" under registration-scale (unparameterized) defaults: this is the live 2026-09-24 zero-themes bug`);
  }
});
...
test('sales-scaled minimums (SALES_ENGINE_OPTIONS) surface the same 4-label construction, including its one-word member, with rise:null (thin/absent reference span)', async (t) => {
  ...
  const row = ['rising', 'new', 'stable', 'fading', 'unranked'].map(b => (tc[b] || []).find(r => r.theme === 'market')).find(Boolean);
  assert.ok(row, 'the 4-label "market" construction must be present somewhere once minimums are scaled to sales input');
  ...
  assert.ok(row.members.includes('market'), 'the single-token (one-word) label must contribute to the theme membership, not be skipped for having fewer than two tokens');
});
...
test('end-to-end: source=sales computeRange surfaces a theme carried by 4 independent built buyers of one construction, including a one-word member, with rise:null', async (t) => {
  ...
  const entries = [
    builtEntry('market.com', 'buyerA'),
    builtEntry('primemarket.com', 'buyerB'),
    builtEntry('harbormarket.net', 'buyerC'),
    builtEntry('trailmarket.io', 'buyerD'),
  ];
  ...
  const theme = result.themes.find(t => t.theme === 'market');
  assert.ok(theme, 'the "market" construction carried by 4 independent built buyers (including the one-word "market" label itself) must be returned for source=sales');
