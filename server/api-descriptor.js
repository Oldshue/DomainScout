...
const ENDPOINTS = [
  ...
  },
  {
    ...
    summary: 'Serves the vendored theme-convergence engine (independence-breadth scoring with single-actor-kit collapse) precomputed from the cloud registration-universe tapes, or (source=sales) from the Sale Watch likely-sale ledger.',
    params: [
      { name: 'source', type: 'string', required: false, description: 'Engine input: "registrations" (default, unchanged) uses the cloud universe lane\'s daily CZDS registration-add tapes. "sales" uses the Sale Watch likely-sale ledger: entries whose tier is verified, probable or suspected AND whose classification is not owner-migration or portfolio-kit (a single-actor bulk-adoption "platform" row), whose reportDate falls within the span; the reference span uses the identical rule over its own dates. "candidates" uses the FULL Sale Watch candidate tape underneath that ledger (the same reader GET /api/sale-watch/candidates pages, over the requested from/to and the reference span, walked end to end server-side rather than sampled): a three-day window holds thousands of built candidates and tens of thousands eligible, far more than any caller can read, so theme convergence over the whole tape is computed here. Default input is BUILT candidates (probed, and the probe found an operating site); see builtOnly.' },
      { name: 'builtOnly', type: 'boolean', required: false, description: 'source=candidates only (a 400 on any other source). Default true: the engine input is built candidates (probed, and the probe found an operating site). false widens the input to every eligible candidate in the window, including unprobed ones.' },
    ...
    response: '200 JSON { range, referenceRange, coverage: { daysPresent, daysMissing, zonesPerDay, comPresent }, referenceCoverage: { daysPresent, daysMissing, zonesPerDay, comPresent } (same shape as coverage, for the reference span every theme\'s `rise` is computed against), riseBasis: "complete-reference" | "partial-reference" (partial when referenceCoverage.daysMissing is non-empty), computedAt, engineVersion, themes: [ { theme, convergence, rise, labels, distinctRoots, distinctConstructions, distinctZones, topZones, kitCollapsed, examples } ] } once computed for source=registrations. For source=sales the top level additionally carries `source: "sales"` and `classifierVersion` (the sale-watch-evidence classifier version the ledger rows were assessed under; a version bump invalidates any stored sales result and forces recompute), and each theme additionally carries `buyers` (count of distinct destination nameserver sets among its likely-sale names), `topBuyerShare` (the largest such set\'s share of the theme\'s names), `builtCount` (names whose destination is an operating site), and `buyerIndependence` (0-1 Gini-Simpson diversity across destination sets; 0 when every name shares one destination regardless of count, approaching 1 across many similarly sized independent destinations) -- sales themes are ranked by buyerIndependence first, then builtCount, then convergence, so a theme carried by many independent built buyers ranks above one carried by a single destination set. For source=sales each example is `{ label, zones, reportDate, tier, fromProvider, toProvider, buyerTitle }` instead of the registrations example shape. For source=candidates the top level additionally carries `source: "candidates"`, `builtOnly` and `inputDigest` (a digest of the candidate reader\'s own window-wide input: per-day ingest receipts and cursor completeness, the eligible census, what the receipts excluded and why, and every batch destination set with its count -- new ingest changes it and invalidates any stored result for the same range), and each theme carries `independentBuyers` (distinct destination nameserver sets, EXCLUDING every set the reader reports as a batch of >=10 names in the window, so one actor moving a block counts as zero independent buyers), `names` and `builtNames` (batch names ARE still listed here), `examples` as `{ domain, departureDay, buyerTitle, destination }` rows diversified across independent destinations first, `rise` versus the reference span, and `batchesCollapsed` (each excluded batch set with its names in this theme and its window-wide count, so the collapse is auditable). Candidate themes are ranked by independentBuyers first, then builtNames, then convergence x rise, so a theme several independent buyers converge on always outranks one actor\'s kit however many names the kit holds. 202 JSON { status: "pending", range, referenceRange, startedAt } while a deduplicated background job computes an on-demand (source, range); poll again until 200.',
  },
  {
  ...
  },
  {
    ...
    params: [],
    response: 'JSON sale-watch ledger projection for the current user. A departure from registrar-default or hosting DNS (never a marketplace/parking listing) straight onto a registrar that mandates its own nameservers (e.g. Cloudflare), with a matching registry transfer to that registrar, is classified `owner-migration` and excluded from verified/probable/suspected: this is an existing owner migrating registrars, not a sale footprint. Prior parked/for-sale site evidence or an observed aftermarket listing can still promote such a row into the ordinary tiers. owner-migration rows remain visible in excludedEntries with a counterEvidence line explaining the exclusion. `coverage.reconstruction.intakeCoverage` is an array of per-day zone-movement intake receipts (most recent first), each `{ day, departures, platformExcluded, topLearnedPlatforms: [{provider, nsKey, count}] (up to 10, highest count first), expiryExcluded, ownerMigrationExcluded, eligible, probed, probeFailures, cursorComplete, tiers: { probable, suspected, transfer } }`: `departures` is that day\'s total zone-movement departures observed; `platformExcluded`/`expiryExcluded` are departures excluded at ingest because their destination nameservers are a known or newly-learned marketplace/parking platform, or a known expiry/suspension lander, respectively (never counted as `eligible`); `eligible` is what remains after platform, expiry, and policy exclusions; `probed`/`probeFailures`/`ownerMigrationExcluded`/`tiers` are counted from the current state of that day\'s candidates, so they grow as later probe waves adjudicate them.',
  },
  {
    path: '/api/sale-watch/candidates',
    summary: 'The full weekly sale-candidate tape: every departure in the window that survived the same platform/parking-batch and expiry exclusions the Sale Watch ledger applies, pageable, before any evidence-tier scoring narrows it.',
    params: [
      { name: 'days', type: 'integer', required: false, description: 'Window length in days, 1-14. Default 7. Ignored when from/to are supplied.' },
      { name: 'from', type: 'string', required: false, description: 'Window start day, YYYY-MM-DD. Must be supplied together with to.' },
      { name: 'to', type: 'string', required: false, description: 'Window end day, YYYY-MM-DD. Must be supplied together with from.' },
      { name: 'q', type: 'string', required: false, description: 'Case-insensitive substring match over the domain and its stored evidence, max 100 chars.' },
      { name: 'tld', type: 'string', required: false, description: 'Restrict rows to one bare suffix, e.g. `com`.' },
      { name: 'built', type: 'boolean', required: false, description: 'true/false restricts to rows a probe has already adjudicated as buyer-built or not. Unprobed rows are unknown and are excluded by either value.' },
      { name: 'limit', type: 'integer', required: false, description: 'Rows per page, 1-1000. Default 500.' },
      { name: 'cursor', type: 'string', required: false, description: 'Opaque pagination.nextCursor from the previous page. It encodes the last (departureDay, cohortSize, domain) plus a digest of the query; a cursor minted for a different query is rejected with 400 instead of silently paging a different result set. There is no offset ceiling, so a caller can page the whole window.' },
    ],
    response: 'JSON { schema: "domainscout.sale-watch-candidates/v1", generatedAt, query, coverage, batches, matched, rows, pagination }. `rows` are ordered newest departureDay first, then cohortSize ascending (independent buyers ahead of bulk moves), then domain; each row carries domain, tld, departureDay + departureDaySource, sellerNameservers + marketplace, destinationNameservers, cohortSize (how many names in the window share that destination nameserver set), probeState (unprobed/probed/probe-failed), built + siteClass + buyerTitle once probed, registrar + transferAt when RDAP has them, and ledgerTier when the name also carries an adjudicated Sale Watch tier. `coverage` is { from, to, departures, eligible, excludedByReason, days } read from the ingest receipts that recorded those exclusions, never re-derived. `batches` lists destination nameserver sets taking >=10 names in the window with their counts, so a caller can collapse a single actor moving a block. Cache-Control: no-store. A failing store read returns 503 { error, detail } and never an empty list; an invalid query or mismatched cursor returns 400.',
  },
  {
...
function llmsText(baseUrl) {
  ...
  lines.push('A name that leaves seller or parking nameservers and serves a built site is an unconfirmed use change, not evidence of an off-market sale. Owner development and private sales landers can produce the same footprint; consult /api/sale-watch for corroborated evidence and exclusions. Use GET /api/universe/ns-movement with selection=departures, from=seller,parking, and state=built to surface exactly those candidates for one universe day:');
