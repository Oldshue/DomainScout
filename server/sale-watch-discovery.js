...
async function discoverSaleLeads({
  ...
  return {
    ...
    coverage: {
      ...
      rdapLookupsFailed: inspected.filter(row => row.discovery?.rdap?.error).length,
      websiteLookupsFailed: inspected.filter(row => row.discovery?.homepage?.error).length,
      pendingTransfers: entries.filter(row => row.tier === 'transfer').length,
