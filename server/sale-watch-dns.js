...
// Discovery-facing parking-class check: expiry landers (EXPIRY_NAMESERVERS
// exact hosts AND EXPIRY_WILDCARD_HOSTS regex hosts, via isExpiryDestination)
// and registrar suspension/verification-hold landers (SUSPENDED_HOST_PATTERN)
// are, for discovery's purposes, the same class of "stopped being a live
// buyer surface" departure as classic parking infrastructure. This reuses
// the existing PARKING_NS_PATTERNS / isExpiryDestination / SUSPENDED_HOST_PATTERN
// checks rather than maintaining a second nameserver table.
// delegationEvidence above still reports expiration/suspended/parking as
// separate finer-grained fields for callers that need the distinction.
function hasParkingClassNameserver(nameservers) {
  const hosts = normalize(nameservers);
  if (hosts.some(ns => PARKING_NS_PATTERNS.some(pattern => pattern.test(ns)))) return true;
  if (isExpiryDestination(hosts).isExpiry) return true;
  if (hosts.some(ns => SUSPENDED_HOST_PATTERN.test(ns))) return true;
  return false;
}

module.exports = { SELLER_NS_PATTERNS, PARKING_NS_PATTERNS, delegationEvidence, hasParkingClassNameserver };
