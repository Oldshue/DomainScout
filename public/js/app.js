...
const app = {
  ...
  knownTakenExtensions(row) {
    ...
    for (const value of Array.isArray(row?.tld_list)
      ? row.tld_list
      : String(row?.tld_list || row?.tlds_list || '').split(',')) addTld(value);
  ...
  openRowTldModal(baseName, rowId, triggerEl) {
    ...
    const knownTlds = this.knownTakenExtensions(row);
    const count = knownTlds.length;
    return this.openTldModal(baseName, count, triggerEl, {
      exact: row?.tlds_verified === true,
      knownTlds,
    });
  },
  ...
  renderRow(d) {
    ...
    const tldsCell = this.extensionCoverageCell(d, baseName);
    return `<tr class="${rowClass}" id="row-${d.id}">
      ...
      <td class="num" id="tld-cell-${d.id}">${tldsCell}</td>
      <td>${age}</td>
      <td>${wb}</td>
  ...
  openModal(id) {
    ...
    document.getElementById('modal-info-grid').innerHTML =
      fmt('TLD', d.tld) +
      fmt('Length', d.length) +
      fmt('Extensions', this.knownTakenExtensions(d).length) +
  ...
  async loadSaleWatch(force = false, append = false) {
    ...
    try {
      ...
      document.getElementById('sale-watch-transfers').textContent = Number(this._saleWatchRows.filter(row=>row.tier==='transfer').length).toLocaleString();
      ...
      if (requestedQuery !== (this._saleWatchQuery || '') || requestedView !== document.getElementById('sale-watch-tier')?.value) {
  ...
  showMoreSaleWatch() {
    ...
    if (this._saleWatchFilteredCount < this._saleWatchVisibleLimit && this._saleWatchLedger?.pagination?.nextCursor != null) this.loadSaleWatch(true, true);
  ...
  renderSaleWatch() {
    ...
    const more = document.getElementById('sale-watch-more');
    if (more) more.hidden = rows.length <= visibleLimit && this._saleWatchLedger?.pagination?.nextCursor == null;
    // Match the visible departure date; later probes must not reorder older moves.
    rows.sort((a,b)=>String(b.reportDate||'').localeCompare(String(a.reportDate||''))||(a.domain<b.domain?-1:a.domain>b.domain?1:0));
    if (status) {
      ...
      const pageWarning = this._saleWatchLedger?.pagination?.nextCursor != null ? ' · more movements available below' : '';
    ...
    list.innerHTML = rows.slice(0, visibleLimit).map(row => `
      <details class="sale-watch-row">
        ...
        <div class="sale-watch-detail">
          ...
          <article><h3>Source and chronology</h3><p>${safe(row.classification === 'reported-sale' ? 'Report date' : 'DNS departure observed')} ${safe(row.reportDate || 'unknown')}<br>${safe(row.observationStatus === 'retained-history' ? `Chronicle retained · last observed ${row.lastObservedAt || 'previous scan'}` : row.observationCount ? `Observed in ${row.observationCount} scan${row.observationCount === 1 ? '' : 's'}` : row.classification === 'reported-sale' ? 'Reported evidence' : 'Dated movement evidence')}<br>${row.sourceUrl ? `<a href="${safe(row.sourceUrl)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer">Open source evidence ↗</a>` : 'Public evidence not linked'}</p></article>
        </div>
      </details>`).join('');
  },
  ...
  async openTldModal(baseName, tldCount, triggerEl, evidenceState = {}) {
    ...
    renderKnownEvidence(seededTlds);
